  ...
  for (const line of lines) {
    ...
    try {
      ...
      if (response.result && response.result.tools) {
        console.log(
          `[PASS] ListTools returned ${response.result.tools.length} tools.`,
        )
        ...
        if (response.error) {
          console.log(
            '[PASS] search_location responded (API Error expected with fake key):',
          ...
          console.log(
            '[PASS] search_location handled API error gracefully:',
            text,
          )
        } else {
          console.log(
            '[PASS] search_location returned success (unexpected with fake key but protocol valid).',
          )
        }
        server.kill()
        process.exit(0)
